...
//
// Параметры:
// 	АдесФайлаИлиМассивАдресов	-	Массив, Строка - Массив с адресами файлов для подключения,
// 													 или строка с одним адресом
//  Поле  						- ПолеФормы - Поле формы с видом ПолеHTMLДокумента 
//
Процедура ПодключитьСрипт(Знач АдесФайлаИлиМассивАдресов, Знач Поле) Экспорт 

	МассивАдресов = Новый Массив();
	Если ТипЗнч(АдесФайлаИлиМассивАдресов) = Тип("Строка") Тогда
		МассивАдресов.Добавить(АдесФайлаИлиМассивАдресов);			
	Иначе
		МассивАдресов = АдесФайлаИлиМассивАдресов;
	КонецЕсли;
	...
	Для Каждого текАдрес Из МассивАдресов Цикл
		...
		ДобавляемыйЭлемент.Источник = текАдрес;
...
Процедура ПодключитьРасширенныеСкрипты(Поле) Экспорт 
	...
	МассивАдресов = Новый Массив;
	МассивАдресов.Добавить(пс_СкриптыСерверПовтИсп.АдресОсновногоРасширенияСкриптов());
	ПриОпределенииПодключаемыхФайлов(МассивАдресов, Поле);
	ПодключитьСрипт(МассивАдресов, Поле);
