...
Процедура ПодключитьАМДЗагрузчик(Поле) Экспорт
	
	ПодключитьСрипт(пс_СкриптыСерверПовтИсп.АдресФайлаАМДЗагрузчика(), Поле);
	
КонецПроцедуры

Процедура ПодключитьСкриптыРедактораМонако(Поле) Экспорт
	
КонецПроцедуры

Процедура СоздатьРедакторКодаМонако(ВнешнийОбъектКонтейнер, Поле) Экспорт
...
Процедура ИнициализироватьКонсольКода(Поле) Экспорт
	АдресБиблиотеки = пс_СкриптыВызовСервера.ПолучитьИзСерверногоКэша("АдресаБиблиотек.monaco");
	Форма = НайтиФормуЭлемента(Поле);
	Выполнить(СтрШаблон("Форма.%1 = ""%2""",Поле.ПутьКДанным, АдресБиблиотеки + "/index.html"));
КонецПроцедуры
