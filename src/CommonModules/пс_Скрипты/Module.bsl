...
Процедура ИнициализироватьРедакторМонако(Поле) Экспорт
	
	АдресБиблиоткиРедактора = пс_СкриптыВызовСервера.ПолучитьИзСерверногоКэша("АдресаБиблиотек.пс_monaco");
	//@skip-warning
	текстХТМЛ = АдресБиблиоткиРедактора + "editor.html";
	Выполнить(СтрШаблон("Форма = НайтиФормуЭлемента(Поле);Форма.%1 = текстХТМЛ", Поле.ПутьКДанным));
