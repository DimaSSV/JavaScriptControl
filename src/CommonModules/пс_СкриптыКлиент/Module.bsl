...
Функция ПолучитьОбработчикиПолученияПараметров()
	...
	Результат.Вставить("АдресАрхиваРедактораКода", "пс_СкриптыСерверПовтИсп.АдресАрхиваРедактораКода");
	Возврат Результат;
...
// Описание
// 
Процедура РаспаковатьИсходникиРедактора()
	АдресАрхива = ПолучитьИзКэша("АдресАрхиваРедактораКода");
	Если АдресАрхива = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Параметры = Новый Структура;
	Параметры.Вставить("АрхивРедактораКода", ПолучитьИзВременногоХранилища(АдресАрхива));
	Описание = Новый ОписаниеОповещения("РаспаковатьИсходникиРедактораПослеПодученияКаталогаВременныхФайлов"
	, ЭтотОбъект
	, Параметры);
...
Процедура РаспаковатьИсходникиРедактораПослеПодученияКаталогаВременныхФайлов(ИмяКаталогаВременныхФайлов
	, ДополнительныеПараметры) Экспорт
	ДополнительныеПараметры.Вставить("ИмяКаталогаФайловРедактора", ИмяКаталогаВременныхФайлов + "Monaco1C");
	Описание = Новый ОписаниеОповещения("РаспаковатьИсходникиРедактораПослеПроверкиСуществованияКаталога"
	, ЭтотОбъект
	, ДополнительныеПараметры);
	КаталогРедактора = Новый Файл(ДополнительныеПараметры.ИмяКаталогаФайловРедактора);
	КаталогРедактора.НачатьПроверкуСуществования(Описание);	
КонецПроцедуры

Процедура РаспаковатьИсходникиРедактораПослеПроверкиСуществованияКаталога(Существует
	, ДополнительныеПараметры) Экспорт
	Если Существует Тогда
		Описание =Новый ОписаниеОповещения("РаспаковатьИсходникиРедактораПослеОчисткиКаталога"
		, ЭтотОбъект
		, ДополнительныеПараметры);
		НачатьУдалениеФайлов(Описание, ДополнительныеПараметры.ИмяКаталогаФайловРедактора);
	Иначе
		#Если Не ВебКлиент Тогда
		Описание = Новый ОписаниеОповещения("РаспаковатьИсходникиРедактораПослеСозданияКаталога"
		, ЭтотОбъект
		, ДополнительныеПараметры);
		НачатьСозданиеКаталога(Описание, ДополнительныеПараметры.ИмяКаталогаФайловРедактора)
		#КонецЕсли				
	КонецЕсли;
КонецПроцедуры

Процедура РаспаковатьИсходникиРедактораПослеСозданияКаталога(ИмяКаталога, ДополнительныеПараметры) Экспорт
	РаспаковатьИсходникиРедактораЗавершение(ДополнительныеПараметры);
КонецПроцедуры

Процедура РаспаковатьИсходникиРедактораПослеОчисткиКаталога(ДополнительныеПараметры) Экспорт
	РаспаковатьИсходникиРедактораЗавершение(ДополнительныеПараметры);	
КонецПроцедуры

Процедура РаспаковатьИсходникиРедактораЗавершение(Параметры)
	Поток = Параметры.АрхивРедактораКода.ОткрытьПотокДляЧтения();
	#Если Не ВебКлиент Тогда
	Чтение = Новый ЧтениеZipФайла(Поток);
	Чтение.ИзвлечьВсе(Параметры.ИмяКаталогаФайловРедактора);
	пс_СкриптыВызовСервера.ПоместитьВСерверныйКэш("АдресРедактораКода", Параметры.ИмяКаталогаФайловРедактора + "/" + "index.html");		
	#КонецЕсли
